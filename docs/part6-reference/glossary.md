...
**client (MCP)** — The protocol machinery inside a host application that talks to a single MCP server. A host connected to three servers runs three clients. The client also owns the agent loop and translates between MCP and the model API. → [What problem MCP solves](../part3-mcp/why-mcp.md)
...
**elicitation** — A server-initiated request for information from the user. It is the one client feature the current MCP specification defines, and the fourth answer to "who invokes this?" after tools, resources, and prompts. It travels as a multi round-trip request, not a server-to-client call. The server returns `input_required`, the client asks the user, and the client retries the original request with the answer. A declined elicitation is a normal outcome, not an error. → [Tools, resources, and prompts](../part3-mcp/primitives.md)
...
**multi round-trip request (MRTR)** — How an MCP server gets information mid-call without opening a request back to the client. The server returns a result with `resultType: "input_required"`, an `inputRequests` map naming what it needs, and an opaque `requestState`. The client gathers the input, then retries the *original* request under a new JSON-RPC id, carrying `inputResponses` and the untouched state. The interaction is two-way. The protocol stays stateless. → [The wire protocol](../part3-mcp/wire-protocol.md)
...
**resultType** — A field required on every MCP result since revision 2026-07-28, with two values: `"complete"` for an ordinary finished result, and `"input_required"` for the interim result that parks a call while the client gathers input. Results from earlier-revision servers omit it and must be read as `"complete"`. → [The wire protocol](../part3-mcp/wire-protocol.md)
...
**server (MCP)** — The program on the other end of an MCP connection, exposing capabilities — tools, resources, prompts — to whichever compliant client calls it. → [What problem MCP solves](../part3-mcp/why-mcp.md)

**server/discover** — The one RPC every MCP server must implement. It returns the server's supported protocol versions, capabilities, identity, and optional `instructions`, all in a single request. Calling it is optional for clients, which may fire any request directly and handle a version error instead. But on stdio there are no HTTP status codes, so it doubles as the probe that tells a modern server from a handshake-era one. → [The wire protocol](../part3-mcp/wire-protocol.md)
...
**session (MCP)** — *Removed from the protocol as of revision 2026-07-28.* Formerly an identifier that a Streamable HTTP server issued at initialization. The client attached it to later requests, tying independent HTTP calls to one logical conversation. Sessions made every request stateful, and remote servers hard to scale, so they were deleted rather than fixed. State that must cross calls now travels as an explicit server-minted handle in tool arguments. → [Transports](../part3-mcp/transports.md)
...
**stateless mode** — Once an opt-in, now simply how MCP works. The server treats every request as self-contained, holding no per-client state between calls. Any replica behind a load balancer can answer any request, and restarts break no clients. → [Transports](../part3-mcp/transports.md)
