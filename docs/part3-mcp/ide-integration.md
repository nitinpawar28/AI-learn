...
Part 3 has zoomed steadily inward. Why a protocol exists, what it carries, how the bytes move, how a server is built.

This chapter is the *used* end of that zoom, where a server you can run in a terminal becomes a tool an assistant drives inside your editor.

By the end you will be able to wire any [stdio](transports.md) MCP server into the four most common clients. You will know the one configuration gotcha that costs the most debugging hours, how to keep secrets out of committed config, and how to diagnose the signature symptom of a misconfigured server: nothing.

## What a client does with your entry

An MCP server ships no installer and registers nothing with the operating system.

Integration is one JSON entry in a client's config file. The entry says only two things: what to call this server, and how to start it — a command, its arguments, and optional environment variables.

The client does the rest mechanically. At startup it spawns the command as a subprocess. It calls [`tools/list`](wire-protocol.md) over the subprocess's stdin and stdout, optionally probing with `server/discover` first. From then on it attaches the returned [tool](primitives.md) definitions to every request it sends to the model.

The client performs all the work that makes it look like the model ["knows"](../part1-fundamentals/what-llms-do.md) about your server.

That chain has a consequence for debugging. Every link is invisible until the last one.

The file could be in the wrong place. The key could be wrong. The spawn could fail. The first request could die. The visible symptom is identical in every case: the tools never appear.

The second half of this chapter is about exactly that.
...
Here is one entry in all four dialects, using the official reference filesystem server. The hands-on at the end of this chapter installs it for real.
...
Secrets follow directly.

A committed config must never contain a literal API key. It would sit in the repository's history forever.

Every dialect above accepts an `env` object on the entry, which sets environment variables for the spawned subprocess. So put the variable's *name* in the committed file, and its *value* in your local environment or user-scoped config.

This is the pattern [transports](transports.md) established. Local stdio servers take credentials from the environment, not from an OAuth flow. Auth follows transport, and [safety](../part4-agents/safety.md) picks up the thread.

## The debugging ladder

When the tools never appear, resist opening the client's settings UI first.

Climb this ladder in order. Each rung is cheap, and each one decisively rules out a whole layer.

1. **Does the server run standalone?** Copy the exact `command` and `args` into a terminal and run them. A healthy stdio server prints nothing and sits waiting on stdin — the same "it waits, and that's correct" behavior from the [transports hands-on](transports.md). A crash or a missing runtime here is a server problem, and no client config can fix it.
2. **Is stdout clean?** stdout is the wire. One stray print statement corrupts the JSON-RPC [framing](wire-protocol.md), and the first request fails to parse. All logging must go to stderr.
...
!!! example "In the wild: Sankshep"
    ...
    One server, four editors, zero code changes. That is the N+M promise from [why MCP](why-mcp.md), landing in config files.
...
??? success "Answer"
    It is the cheapest decisive test. It needs no client at all, and it splits the world in two.

    If the process crashes or the runtime is missing, no config change can help, and the client logs would only report the same failure less directly.

    If it starts and waits silently, the server side is healthy. So the fault must sit in framing, config, or the client — which the later rungs check in order.
...
??? success "Answer"
    ...
    Committed config describes the project. Secrets describe, and belong to, the user.

**4.** Trace what a client does with a config entry, between application launch and the first successful tool call in chat.

??? success "Answer"
    It reads the config file. It spawns the entry's command as a subprocess. It calls `tools/list` over the subprocess's stdio, and attaches the returned tool definitions to each request it sends to the model.

    When the model emits a tool call, the client maps it to `tools/call`, runs it against the server — asking the user for approval where required — and returns the result into the conversation.

    That is the full loop from [the wire protocol](wire-protocol.md).
...
5. In chat, ask: "List the files in the sandbox directory, then create `notes.txt` there containing one line: hello from MCP."
    ...
    Then confirm `notes.txt` exists on disk.

If step 4 or 5 fails, match your symptom:

??? failure "Nothing appears at all"
    Wrong top-level key for this client — the gotcha — or the wrong file location, or the client was not fully restarted.
...
??? failure "The server never starts"
    ...
    Typical causes: Node.js not installed, `npx` not on the client's PATH, or a typo in the package name.

??? failure "Tools appear, but calls error"
    Check the paths in `args`. They must be absolute, and every path you ask about must sit inside an allowed directory.
...
[Build your own MCP server](../part6-reference/build-your-own.md) puts your own code in that subprocess's seat.
